...
void CalendarActivity::loadSyncUrl() {
  ...
  if (read == 0) return;
  buf[read] = '\0';
  syncUrl = normalizeCalendarUrl(std::string(buf.get(), read));
}
...
void CalendarActivity::configureSyncUrl() {
  startActivityForResult(
      ...
      [this](const ActivityResult& result) {
        ...
        statusMessage = fetchRemoteCalendar() ? "Calendario sincronizzato" : "Sync non riuscito";
        loadEvents();
        requestUpdate(true);
      });
}
...
void CalendarActivity::loop() {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) configureSyncUrl();
}
