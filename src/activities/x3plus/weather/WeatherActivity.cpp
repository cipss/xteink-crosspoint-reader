...
bool WeatherActivity::fetchWeather() {
  ...
  const auto maxTemps = daily["temperature_2m_max"];
  const auto minTemps = daily["temperature_2m_min"];
  const auto codes = daily["weather_code"];
  if (times.isNull() || maxTemps.isNull() || minTemps.isNull() || codes.isNull()) return false;

  for (int i = 0; i < FORECAST_DAYS; ++i) {
    if (times.size() <= static_cast<size_t>(i) || maxTemps.size() <= static_cast<size_t>(i) ||
        minTemps.size() <= static_cast<size_t>(i) || codes.size() <= static_cast<size_t>(i)) {
      forecast[i] = {};
      continue;
    }
    forecast[i].date = times[i].as<const char*>();
...
void WeatherActivity::render(RenderLock&&) {
  ...
  if (!configured) {
    renderer.drawCenteredText(UI_12_FONT_ID, height / 2 - 30, "Weather non configurato");
    renderer.drawCenteredText(UI_10_FONT_ID, height / 2 + 5, "/.crosspoint/x3plus/weather.cfg");
    renderer.drawCenteredText(UI_10_FONT_ID, height / 2 + 28, "latitude=... longitude=...");
  } else if (loading) {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, height / 2 - 110, temp.c_str());
    renderer.drawCenteredText(UI_12_FONT_ID, height / 2 - 72, describeWeatherCode(weather.weatherCode));
    renderer.drawCenteredText(UI_10_FONT_ID, height / 2 - 42, feels.c_str());
    renderer.drawCenteredText(UI_10_FONT_ID, height / 2 - 18, humidity.c_str());

    const int rowY = height / 2 + 12;
    for (int i = 0; i < FORECAST_DAYS; ++i) {
      if (forecast[i].date.empty()) continue;
      const std::string date = forecast[i].date.size() >= 10 ? forecast[i].date.substr(5) : forecast[i].date;
      const std::string line = date + "  " + describeWeatherCode(forecast[i].weatherCode) +
                               "  " + std::to_string(static_cast<int>(forecast[i].minTemperature + 0.5f)) +
                               "/" + std::to_string(static_cast<int>(forecast[i].maxTemperature + 0.5f)) + " C";
      renderer.drawText(SMALL_FONT_ID, 12, rowY + i * 28, line.c_str());
    }
  }
